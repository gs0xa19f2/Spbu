...
def tsp(cityCoordinates, popSize=100, maxGeneration=300):
    MUTATION_RATE = 60
    ...
    def GeneticAlgorithm(popSize, maxGeneration):
        ...
        # Visualize
        # print("Generation: {0}\nPopulation Size: {1}\t Average Fitness: {2}\nBest Fitness: {3}"
                #   .format(generation, len(population), averageFitness, bestGenome.fitness))
        print("Best fitness", bestGenome.fitness)
        print(bestGenome.chromosomes)
        plot(generation, allBestFitness, bestGenome, cityCoordinates)

    GeneticAlgorithm(popSize=popSize, maxGeneration=maxGeneration)
...
if __name__ == "__main__":
    ...
    tsp(cityCoordinates, popSize=100, maxGeneration=300)
